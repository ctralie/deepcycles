...
class RipsRegularizedAutoencoderCNN(nn.Module):
    def __init__(self, device, x, depth, dim, win, lam=1, lr=1e-2, last_layer = nn.Sigmoid):
        """
        Parameters
        ----------
...
class BlockRipsRegularizedAutoencoderCNN(nn.Module):
    def __init__(self, device, x, depth, dim, win, block_win, block_hop, lam=1, lam_block=1, lr=1e-2, last_layer = nn.Sigmoid):
        """
        ...
        ## Step 3: Create layers for sliding window of overall video
        self.batch_norm = nn.BatchNorm2d(3)
        self.vid_dist = SlidingVideoDistanceMatrixLayer(win, device)
        self.rips = RipsPersistenceDistance([1])

        ## Step 4: Create layers for sliding window of blocks
        self.blocks = []
        self.block_losses = []
        n_frames = x.shape[0]
        i = 0
    ...
    def forward(self):
        ...
        for (batch_norm, vid_dist, rips) in self.blocks:
            ...
            i += self.block_hop
        ## Step 3: Compute overall persistence
        z = self.batch_norm(y)
        D = self.vid_dist(z)
        dgms = self.rips(D.cpu())
        return y, block_Ds, block_dgms, D, dgms
    
    def train_step(self):
        self.optimizer.zero_grad()
        self.train()
        y, block_Ds, block_dgms, D, dgms = self.forward()
        mse_loss = self.mse_loss(self.x, y)
        dgm_score, largest_pers, skip1_sum_h1_loss = self.dgm_loss.get_loss(dgms)
        ...
        loss = mse_loss+ self.lam_block*block_stdev - self.lam_block*torch.mean(block_losses) - self.lam*dgm_score 

        loss.backward()
        self.optimizer.step()
        self.losses.append(loss.item())
        return y, D, block_Ds, dgms, mse_loss.item(), self.lam*largest_pers.item(), self.lam*skip1_sum_h1_loss.item(), block_losses, block_stdev.item()
    ...
    def train_epochs(self, num_epochs, plot_interval=0):
        ...
        block_means = []
        for epoch in range(num_epochs):
            y, D, block_Ds, dgms, mse_loss, largest_pers, sum_other_persistence, block_losses, block_stdev = self.train_step()
            block_losses = block_losses.cpu().detach().numpy()
            ## Everything below this point is just for plotting
            largest_pers *= self.lam
            sum_other_persistence *= self.lam
            mse_losses.append(mse_loss)
            largest_perses.append(largest_pers)
            sum_other_persistences.append(sum_other_persistence)
            block_stdevs.append(block_stdev*self.lam_block)
            ...
            if plot_interval > 0 and (plot_idx%plot_interval == 0 or plot_idx == num_epochs-1):
                plt.clf()
                plt.subplot2grid((2, 5), (0, 0), colspan=3, rowspan=2)
                plt.title("Epoch {}: MSE Loss {:.6f}, DGM Score: {:.6f}, Block STDev: {:.6f}, Block Means: {:.6g}, Total Loss: {:.6f}".format(epoch, mse_loss, dgm_score, block_stdev, block_means[-1], self.losses[-1]))
                plt.plot(mse_losses)
                plt.plot(largest_perses)
                plt.plot(sum_other_persistences)
                plt.plot(block_stdevs)
                plt.plot(block_means)
                plt.legend(["MSE Loss", "Largest Persistence", "Sum of Other Persistences", "Block STDevs", "Block Means"])
                plt.scatter([epoch], [mse_loss], c='C0')
                plt.scatter([epoch], [largest_pers], c='C1')
                plt.scatter([epoch], [sum_other_persistence], c='C2')
                plt.scatter([epoch], [block_stdev*self.lam_block], c='C3')
                plt.scatter([epoch], [block_means[-1]], c='C4')
